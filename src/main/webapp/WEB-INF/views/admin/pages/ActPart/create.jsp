...
<div class="panel panel-success">
    ...
    <div class="panel-heading  text-center text-primary">Act Part<span class="pull-right"><a href="#ActPart/index"
                                                                                             role="button"> <i
            class="fa fa-history" aria-hidden="true"></i>
Back to List</a></span></div>
    <div class="panel-body">
        <form:form method="post" role="form" modelAttribute="actPart"
                   action="${contextPath}/admin/actPart${action}?${_csrf.parameterName}=${_csrf.token}"
                ...
                <div class="form-group ${(not empty status.errorMessage) ? ' has-error':''}">
                    ...
                    <div class="col-sm-9">
                        <form:select class="form-control"
                                     data-change-pull="actId.id|volume.id|${contextPath}/admin/actPart/act-list"
                                     data-selected="${actPart.volume.id}" data-itemLabel="volumeName"
            ...
            <div class="form-group ${(not empty status.errorMessage) ? ' has-error':''}">
                ...
                <div class="col-sm-9">
                    <form:select class="form-control" data-selected="${actPart.actId.id}" data-itemLabel="shortTitle"
            ...
            <div class="form-group ${(not empty status.errorMessage) ? ' has-error':''}">
                ...
                <div class="col-sm-9">
                    <form:input type="text" class="form-control"
                                data-validation="trim|required|min_length[1]|max_length[255]" id="inputpartNo"
                                path="partNo" placeholder="Part No"/>
                    <form:errors path="partNo" cssClass="help-block text-danger" element="span"/>
                </div>
            </div>
        </spring:bind>
            <jsp:include page="/WEB-INF/views/admin/pages/ActPart/includes/partNoFootnote.jsp"/>
            <spring:bind path="partName">
                <div class="form-group ${(not empty status.errorMessage) ? ' has-error':''}">
                    <label for="inputpartName" class="col-sm-2 control-label">Part Name</label>
                    <div class="col-sm-9">
                        <form:input type="text" class="form-control"
                                    data-validation="trim|required|min_length[1]|max_length[255]" id="inputpartName"
                                    path="partName" placeholder="Part Name"/>
                        <form:errors path="partName" cssClass="help-block text-danger" element="span"/>
                    </div>
                </div>
            </spring:bind>
            <jsp:include page="/WEB-INF/views/admin/pages/ActPart/includes/partNameFootnote.jsp"/>
            <spring:bind path="partOrder">
                <div class="form-group ${(not empty status.errorMessage) ? ' has-error':''}">
                    <label for="inputpartOrder" class="col-sm-2 control-label">Part Order</label>
                    <div class="col-sm-9">
                        <form:input type="number" class="form-control" data-validation="trim|required|integer"
                                    id="inputpartOrder" path="partOrder" placeholder="Part Order"/>
