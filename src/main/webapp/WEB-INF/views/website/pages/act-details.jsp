...
<a target="_blank" href="${contextPath}/act-print-${act.id}<%--/section-print-${section.id}--%>.html" class="pull-right no-print" style="width: 72px;height: 80px;margin-right: -40px;
...
<div class="col-md-11 boxed-layout <c:if test="${hl!=null}">searchable</c:if>" style="padding: 10px;width: 97%">
                ...
                <div class="col-sm-3 txt-head" style="width: 18%;float: left;">
                    <c:choose>
                        ...
                        <c:when test="${fn:contains(section.sectionName, '।')}">
                            ...
                            <ata:footnote text="${string4}" footnoteNo="${footnoteNo}" count="count"
                                          locale='${act.isBanglaAct==true? "bn":"en"}' footnoteTotal="footnoteTotal"
                                          footnotes="${section.sectionNameFootnoteList}"/>
                            <c:set var="footnoteNo" value="${count}"/>
                            <c:set var="footnoteList">${footnoteList} ${footnoteTotal}</c:set>
