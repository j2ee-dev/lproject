...
<%--<button class="printIco pull-right" style="width: 72px;height: 80px;" id="btnPrint"><i
        class="fa fa-print" style="font-size: 40px;"></i></button>--%>

<c:forEach items="${sections}" var="section" begin="0" end="0" varStatus="loop">
    <a target="_blank" href="${contextPath}/act-${act.id}/act-chapter-print-${section.chapterId.id}<%--/section-print-${section.id}--%>.html" class="pull-right no-print" style="width: 65px;height: 80px;margin-right: -65px;
            ...
            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                <div class="text-center">
                    <h3>
                        ...
                        <ata:footnote text="${act.shortTitle}" footnoteNo="${footnoteNo}" count="count" locale='${act.isBanglaAct==true? "bn":"en"}' footnoteTotal="footnoteTotal" footnotes="${act.shortTitleFootnoteList}"/>
    ...
    <section class="padding-bottom-20 padding-top-20 bg-striped">
        ...
        <c:forEach items="${sections}" var="section" varStatus="loop">
            ...
            <div class="row <c:if test="${act.repealedAct.status==true}"><c:out value="${act.isBanglaAct==true? 'bn-repealed':'repealed'}"/></c:if>">
                    ...
                    <div class="act-part-group head">
                        <c:if test="${ fn:trim(section.partId.partNo)!=''}">
                            <p class="act-part-no">
                                ...
                                <a href="${contextPath}/act-${section.actId.id}/part-${section.partId.id}.html">
                                    <ata:footnote text="${partNo}" footnoteNo="${footnoteNo}" count="count" locale='${act.isBanglaAct==true? "bn":"en"}' footnoteTotal="footnoteTotal" footnotes="${section.partId.partNoFootnoteList}"/>
                                </a>
                        ...
                        <c:if test="${ fn:trim(section.partId.partName)!=''}">
                            <p class="act-part-name">
                                ...
                                <a href="${contextPath}/act-${section.actId.id}/part-${section.partId.id}.html">
                                    <ata:footnote text="${section.partId.partName}" footnoteNo="${footnoteNo}" count="count" locale='${act.isBanglaAct==true? "bn":"en"}' footnoteTotal="footnoteTotal" footnotes="${section.partId.partNameFootnoteList}"/>
                                </a>
                                <c:set var="footnoteNo" value="${count}"/>
                                <c:set var="footnoteList">${footnoteList} ${footnoteTotal}</c:set>
                            </p>
                    ...
                    <div class="act-chapter-group">
                        <c:if test="${fn:trim(section.chapterId.chapterNo)!=''}">
                            <p class="act-chapter-no">
                                ...
                                <ata:footnote text="${chapterNo}" footnoteNo="${footnoteNo}" count="count" locale='${act.isBanglaAct==true? "bn":"en"}' footnoteTotal="footnoteTotal" footnotes="${section.chapterId.chapterNoFootnoteList}"/>
                                <c:set var="footnoteNo" value="${count}"/>
                                <c:set var="footnoteList">${footnoteList} ${footnoteTotal}</c:set>
                            </p>
                        </c:if>
                        <c:if test="${fn:trim(section.chapterId.chapterName)!=''}">
                            <p class="act-chapter-name" style="font-weight: bold">
                                ...
                                <ata:footnote text="${section.chapterId.chapterName}" footnoteNo="${footnoteNo}" count="count" locale='${act.isBanglaAct==true? "bn":"en"}' footnoteTotal="footnoteTotal" footnotes="${section.chapterId.chapterNameFootnoteList}"/>
                                <c:set var="footnoteNo" value="${count}"/>
                                <c:set var="footnoteList">${footnoteList} ${footnoteTotal}</c:set>
                            </p>
                        </c:if>
                ...
                <c:if test="${section.sectionHead!=null || fn:trim(section.sectionHead)!=''}">
                    ...
                    <c:if test="${section.sectionHead!=sectionHead}">
                        <p class="act-section-head text-center bd-info margin-top-20">
                            <ata:footnote text="${section.sectionHead}" footnoteNo="${footnoteNo}" count="count" footnotes="${section.sectionHeadFootnoteList}" locale='${act.isBanglaAct==true? "bn":"en"}' footnoteTotal="footnoteTotal"/>
                            <c:set var="footnoteNo" value="${count}"/>
                            <c:set var="footnoteList">${footnoteList} ${footnoteTotal}</c:set>
                        </p>
                    </c:if>
                ...
                <div class="col-sm-9 txt-details act-chapter-details" id="sec-dec">
                    <ata:actLink text="${section.sectionDescription}" contextPath="${contextPath}" output="output" actShortList="${actShortList}"/>
                    <ata:footnote text="${output}" footnoteNo="${footnoteNo}" count="count" footnotes="${section.sectionDescriptionFootnoteList}" locale='${act.isBanglaAct==true? "bn":"en"}' footnoteTotal="footnoteTotal"/>
                    <c:set var="footnoteNo" value="${count}"/>
                    <c:set var="footnoteList">${footnoteList} ${footnoteTotal}</c:set>

                    <ata:actLink text="${section.sectionTable}" contextPath="${contextPath}" output="output" actShortList="${actShortList}"/>
                    <ata:footnote text="${output}" footnoteNo="${footnoteNo}" count="count" footnotes="${section.sectionTableFootnoteList}" locale='${act.isBanglaAct==true? "bn":"en"}' footnoteTotal="footnoteTotal"/>
                    <c:set var="footnoteNo" value="${count}"/>
                    <c:set var="footnoteList">${footnoteList} ${footnoteTotal}</c:set>
                </div>
            </div>
        ...
        <div class="footnoteListAll">
            <ul class="list-unstyled">
                <ata:actLink text="${footnoteList}" contextPath="${contextPath}" output="" actShortList="${actShortList}"/>
            </ul>
        </div>
    </section>
